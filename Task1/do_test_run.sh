...
# Check if model directory exists for optional tarball
if [ -d "${SCRIPT_DIR}/model" ]; then
    MODEL_VOLUME="--volume ${SCRIPT_DIR}/model:/opt/ml/model/:ro"
else
    MODEL_VOLUME=""
fi
...
echo "=+= Expected output: ${OUTPUT_DIR}/images/pancreatic-tumor-segmentation/tumor_seg.mha"

# Check if output was created successfully
if [ -f "${OUTPUT_DIR}/images/pancreatic-tumor-segmentation/tumor_seg.mha" ]; then
