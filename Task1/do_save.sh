...
# Create the model tarball if model directory exists
if [ -d "${SCRIPT_DIR}/model" ]; then
    echo "==+=="
    output_tarball_name="${SCRIPT_DIR}/model.tar.gz"
    echo "Creating the model tarball as ${output_tarball_name}. This can take a while."
    echo ""
    
    tar -czf $output_tarball_name -C "${SCRIPT_DIR}/model" .
    echo "Model tarball was created as ${output_tarball_name}"
    echo "==+=="
else
    echo "==+=="
    echo "Model directory not found. Skipping model tarball creation."
    echo "Models should be included in the container image."
    echo "==+=="
fi